...
//   Use netanim to visualize the experiment
  char animfile[100];
  sprintf(animfile, "./scratch/project/datas/anim-%d-nodes.xml", nNodes); // Save the result in an xml file
  AnimationInterface anim(animfile);
  Simulator::Stop(Seconds(simTime));
  Simulator::Run();

  char filename[100];
  sprintf(filename, "./scratch/project/datas/final-%d-nodes.xml", nNodes);
...
int main(int argc, char *argv[])
{
  ...
  uint32_t nNodes = 30;       // Initialize number of nodes in the network
